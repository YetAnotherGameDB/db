...
On pushes to `main`, after validation passes, the `publish` job in the same workflow builds the site and force-pushes it as a single orphan commit to the `gh-pages` branch, which GitHub Pages serves:
...
The `thumbnails/` directory on `gh-pages` (per-platform cover packs `thumbs-<platform>.zip` + `manifest.json`, polled by the mobile app) is maintained on that branch by an external pipeline. The publish job preserves it when redeploying — never delete it or commit it to `main`.
